...
public class PrizeController {
    ...
    @ApiOperation(value = "根据id查询奖品信息接口，单参数",notes ="输入奖品信息id")
    @GetMapping("/SelectPrizeById")
    public Prize SelectPrizeById(@ApiParam(value = "奖品id", required = true, example = "1")Integer id){
        return prizeService.SelectPrizeById(id);
    }
    @ApiOperation(value = "查询全部广告信息接口，无参数，返回一个数组，数组里是封装了奖品信息的实体对象",notes ="无输入")
    @GetMapping("/selectAllPrize")
    public List<Prize> selectAllPrize(){
        return prizeService.selectAllPrize();
    }
